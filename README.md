...
本项目包含三个 Python 脚本，用于与大学教务系统（例如 http://jw.cupk.edu.cn）进行交互：
...
*   必要的 Python 库:
    *   `requests`
    *   `beautifulsoup4`
    *   `re` (内置库，无需额外安装，主要用于 `pj.py`)

## 安装与配置

1.  **获取脚本**:
    *   克隆本仓库或直接下载 `cjcx.py`, `jw.py`, 和 `pj.py` 文件。
...
2.  **设置定时任务 (可选)**:
    *   您可以为 `cjcx.py` (成绩查询) 和 `jw.py` (课表查询) 设置定时任务，让脚本定期自动运行。
...
*   **登录失败**:
    ...
    *   检查网络连接是否正常。
*   **获取信息失败 (`cjcx.py`, `jw.py`, `pj.py`)**:
    *   可能是教务系统页面结构发生变化，导致 BeautifulSoup 解析失败。需要更新脚本中的解析逻辑。
    *   网络不稳定或请求超时。
*   **PushPlus 推送失败 (`cjcx.py`, `jw.py`)**:
